...
We install a basic NGINX *coffee* app and make it accessible via container port 8080

Now we can start propagating the service on the bigip. 
For this we need to create an ingress service - and with the help of *annotations* add f5 bigip config.
...
This will add a Virtual Server with IP *10.1.10.80*, listenign on port *80*, an using a monitor to check */*. 
...
Now check what happened on the f5 in the backend:
...
We can see the service created. Pool member is the Cluster IP of the POD.

Now try scaling your application::
...
We just added a basic ingress service - and scaled the application from 2 to 3 pods. All this was monitored by the f5 CIS controller and dynamically updated on the bigip.
